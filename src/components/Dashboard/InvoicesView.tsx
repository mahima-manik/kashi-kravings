    ...
    <div className="space-y-6">
      ...
      <div className="bg-surface-card rounded-xl border border-surface-border p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-white">Upload Invoices</h2>
        </div>
        ...
        >
          ...
          <p className="text-gray-500 text-xs">MyBillBook invoice report (.csv)</p>
          <input
            ref={fileInputRef}
            type="file"
            accept=".csv"
              ...
              <tbody className="divide-y divide-surface-border">
                {filteredInvoices.map((inv) => (
                  <tr key={inv.invoiceNo} className="hover:bg-surface-card-hover transition-colors">
                    ...
                    <td className="px-4 py-3 text-gray-300">{inv.contactName}</td>
                    <td className="px-4 py-3 text-white text-right">{formatCurrency(inv.amount)}</td>
                    <td className="px-4 py-3 text-right">
                      <span className={inv.remainingAmount > 0 ? 'text-amber-400' : 'text-gray-500'}>
